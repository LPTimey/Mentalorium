...
\title[AI Fallibility \& \gls{automationbias} long-term Effects]{The Long-Term Effects of Indicating \Gls{ai} Fallibility on \gls{automationbias} in Mental Health Application Users}
...
They then received case vignettes describing a mental health scenario before submitting a prompt requesting an assessment of the presented case data.

After reviewing the generated response, they subsequently completed a post-interaction questionnaire.
...
The results suggest that disclaimers may reduce \gls{automationbias}, although the effect does not appear to persist over time.
However, a rebound effect occurred in which \glspl{participant} from the active group exhibited higher \gls{automationbias} scores in the second session.
On the other side, the control group showed a notable shift in answers in session 2, surprisingly showing a lower \gls{automationbias} score compared to session 1.
\end{abstract}

\input{ccs}

\keywords{Mental Health, AI, Artificial Intelligence, FWS, STUD, \gls{automationbias}, Fallibility, Mental Health Application, \Gls{ai} Assistant, Long-Term, Long-Term Effects, Priming, Trust, Confidence}
...
As AI-based applications become increasingly accessible to the general population, they may also help alleviate some of the pressure on already overburdened healthcare systems.
...
One such phenomenon is \gls{automationbias}, which describes the tendency of users to over-rely on automated systems and accept their recommendations without sufficient critical evaluation.
...
To address this research gap, the present study investigates human \gls{automationbias} in the use of \Gls{ai} for mental health support.
...
Previous research by \citeauthor*[]{wingerter_mitigating_2025} showed that warning nudges communicating the fallibility of \Gls{ai} can mitigate \gls{automationbias} to a certain extent.
...
Consequently, this study examines the impact of warning nudges—in the form of disclaimers—on users’ reliance on \Gls{ai} within a mental health context \autocite[]{wingerter_mitigating_2025}.
...
Machine learning algorithms already show efficacy in classifying mental disorders and treatment responses, which suggests a promising potential \autocite[]{gonzalez_systematic_review}.
% Nevertheless, it raises concerns with accessibility, data security, and cultural diversity.
% It is also known that biases in the algorithm can have negative influences \autocite[]{ali_artificial_2025}.
% However, human biases in the use of \Gls{ai} for mental health has not yet been covered by previous research.
% Since the human factor in the use of \Gls{ai} in mental health shall not be ignored in such sensitive issues, we dedicated this study to exploring human \gls{automationbias} in the usage of \Gls{ai} in mental health.
...
\gls{automationbias} describes the tendency of users to place excessive trust in automated recommendations and insufficiently verify their correctness \autocite[p~391]{parasuraman_complacency_2010}.
...
Their results demonstrated that participants exposed to faulty \Gls{ai} advice performed significantly worse than participants without \Gls{ai} support, indicating a substantial \gls{automationbias} effect \autocite[]{wingerter_mitigating_2025}.
...
The warning nearly doubled performance relative to the faulty-\gls{ai} condition and reduced \gls{automationbias} to a level comparable to the control group.
Interestingly, self-reported \Gls{ai} literacy did not significantly reduce susceptibility to \gls{automationbias}, suggesting that interface design interventions may be more effective than user experience or prior \Gls{ai} knowledge alone.


While \citeauthor[]{wingerter_mitigating_2025} demonstrate that warning nudges can reduce \gls{automationbias} during a single interaction with generative AI, their study does not investigate whether such effects persist over time.
...
By combining research on AI-supported mental health applications with findings on \gls{automationbias} mitigation through nudging, this work addresses an important gap in the current literature.
...
% To formally characterize the participant allocation procedure, we introduce a set-theoretic representation of the experimental design.
% The following definitions specify the available groups, case vignettes, truthfulness conditions, rounds, and sessions, together with the constraints that determine whether a participant allocation is valid.
...
Furthermore, let $S$ denote the set of all valid sessions consisting of three rounds such that no \gls{casevignette} occurs twice within a session and only one is truthful.
Also, each session pair $(s_1,s_2) \in S^2$ is only allowed to share exactly one \gls{casevignette}.
...
\begin{itemize}
    \item \(P\) denoting the set of valid \glspl{participant} defined previously in Eq.~\eqref{eq:Sets},
    \item \(n \in \mathbb{N}_0\) denoting the participant index,
    \item \(p_n \in P\) denoting the generated \glspl{participant} corresponding to the index \(n\),
...
For the purposes of this study, we define a heuristic \gls{automationbias}-score $B$ that serves as an operational measure of \gls{automationbias}.
$B$ is modelled as a continuous function that maps evaluation parameters to a non linear scalar value in the interval $[0,1]$, representing the degree of evidence that \gls{automationbias} has occurred.
...
In the context of
\gls{automationbias}, higher ratings for incorrect answers indicate a stronger
...
% \subsubsection{Threshold for Minimal \gls{automationbias}}
...
% &= (1 - 0)
% \cdot \frac{3 - 1}{4}
% \cdot \frac{50}{100} \\begin{equation*}4pt]
% &= 1 \cdot \frac{2}{4} \cdot 0.5 \\begin{equation*}4pt]
% &= 1 \cdot 0.5 \cdot 0.5 \\begin{equation*}4pt]
% &= 0.25.
% \end{aligned}
% \end{equation*}
...
% B(i,r,c) > 0.25
% \end{equation*}
% as indicating evidence of \gls{automationbias}.
...
The primary objective of the study was to investigate the direct and long-term influence of disclaimer messages on participants' evaluations of AI-generated mental-health recommendations and on the occurrence of \gls{automationbias}.
...
The initial quality scale is designed to be the core of our \gls{automationbias} measurement, which would then later be scaled by the subsequent confidence inquiry.
...
We decided to source existing mental-health-related \glspl{casevignette} from medical platforms instead of making up our own, as they provided a much more realistic basis.
On top of that, they already contained appropriate recommendations that we could adopt for the correct \gls{ai} output which we would give to the \glspl{participant}.
We made sure to draw the \glspl{casevignette} from 3 different sources as to guarantee more widely spread diagnoses.
To even further boost case authenticity and increase the \glspl{participant}' empathy towards the patients describes in the vignettes, we modified their visual appearance by adding portraits for the individual patients (see \ref{app:VignettesPictures}).
...
Since the study investigates \gls{automationbias}, the design of the disclaimer elements was of particular importance.
...
They then completed the same AI-attitude questionnaire and proceeded through the three rounds assigned to their second session.
...
\begin{description}
\item[$H_1$] Exposure to a design intervention in a mental health \Gls{ai} application will lead to a substantial reduction in \gls{automationbias} scores, such that participants in the intervention group will exhibit lower \gls{automationbias} scores than participants in the control group.

\item[$H_2$] There will be a prolonged effect of indicating AI-fallibility, so that participants from the active group will exhibit lower \gls{automationbias} scores even after a time gap of 1 week between session 1 and session 2, and without being shown the design intervention again.
\end{description}
...
In general, each of the 3 data points were collected after each \gls{round}, making up a total of 3 samples per session.

\section{Results}
% A Wilcoxon test for our data was conducted, and revealed that our data was normally distributed.
Ein Wilcoxon Signed-Rank Test ergab keinen signifikanten Unterschied zwischen den Messzeitpunkten, W = 117, p = .533, rrb = .152.Therefore we
...
It revealed that there was no difference between the active and control groups throughout both sessions (\(F(1,22)=0.00\), \(p = 0.983\), \(\eta^2p < 0.001\)).
...
Even more interesting is that the active group exhibited a positive trend in \gls{automationbias}, so more \gls{automationbias} in session 2 compared to session 1 \((0.458 \to 0.495)\), while the control group exhibited a negative trend in \gls{automationbias}, so less \gls{automationbias} in session 2 compared to session 1 \((0.562 \to 0.388)\).


\subsection{UEQ+}

We deployed several \gls{ueqplus} scales in order to keep track on potential underlying factors that might have influence on the \glspl{participant}' responses.
...
\subsection{Automation Bias}
One of the core decisions taken in the context of our study is the definition and operationalization of \gls{automationbias}.
Our final approach to this study was informed by the methodology described by \citeauthor[]{kucking_automation_2026}, who measured \gls{automationbias} in agreement with objectively false automated recommendations \autocite[]{kucking_automation_2026}.
...
Because mental health experiences are inherently subjective and often not directly observable \autocite[]{mocnik_multimodal_2025}, the focus of this study was not on behavioral manifestations of \gls{automationbias} but rather on participants' endorsement of automated recommendations.
Subjective endorsement offers insight into the propensity to trust erroneous AI-generated advice, which constitutes a central aspect of \gls{automationbias} \autocite[]{parasuraman_complacency_2010}.
...
A potential limitation of the present study is the operationalization of \gls{automationbias} through participants’ ratings and confidence ratings rather than through an explicit measure of \Gls{ai} recommendation acceptance.
...
Consequently, a direct acceptance question may not necessarily provide a clearer measure of \gls{automationbias}, as responses would depend heavily on how participants interpret the concept of acceptance.
...
Still, it misses the other just as important aspect of \gls{automationbias}, as it does not specifically ask if the advice was accepted.
...
We assume it is possible that this happened because \glspl{participant} that were primed increasingly fixated on automation errors, thus consequently having higher standards for risk handling inside the \gls{ai} text.

\section{Conclusion}
Interestingly, the results are not what we have assumed them to be.
Our assumption that people within the active group would have less \gls{automationbias} was correct, although this effect does not have statistical significance \((p = .463)\).

We also assumed that over time there would be a prolonged, positive effect of disclaimers on \gls{automationbias}.
Our results have proven this to be a false assumption, as people from the active group exhibited noticeably more \gls{automationbias} in session 2 compared to session 1.
Interestingly the control group has exhibited noticeably less \gls{automationbias} in session 2 compared to session 1.
...
These adjustements were made carefully, considering the original content, and aim to optimize the quality and comprehensibility of this work.}\autocite[slide 54]{ai-usage-disclaimer}
\end{acks}
...
\subsubsection{Subpar Answer}
For Martina, stimulants such as methylphenidate (Ritalin) or lisdexamfetamine are well-suited, as they can provide long-term relief from burnout symptoms.
...
\begin{figure}[H]
%     \centering
%     \includegraphics[width=0.5\linewidth]{assets/graphs/\gls{automationbias} graph.png}
%     \caption{Caption}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets//ueq/UEQ_Plus_Control_01.png}
    \caption{Control Group, Session 01, UEQ+ Results}
    \label{fig:UEQ_Plus_Control_01}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets//ueq/UEQ_Plus_Control_02.png}
    \caption{Control Group, Session 02, UEQ+ Results}
    \label{fig:UEQ_Plus_Control_02}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets//ueq/UEQ_Plus_Active_01.png}
    \caption{Active Group, Session 01, UEQ+ Results}
    \label{fig:UEQ_Plus_Active_01}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{assets//ueq/UEQ_Plus_Active_02.png}
    \caption{Active Group, Session 02, UEQ+ Results}
