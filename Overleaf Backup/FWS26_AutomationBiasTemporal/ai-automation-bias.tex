...
Results indicate that disclaimers may reduce automation bias initially, but this effect did not persist over time. In Session 2, a rebound effect emerged, with the active group showing higher \sym{abs}.
...
\glslink{rounds}{}
\glslink{sessions}{}
\glslink{session-pairs}{}
\glslink{participants}{}


Let \gls{groups} be the set of all possible groups participants \gls{participants} can be assigned to,
\gls{cases} the set of all possible case vignettes,
\gls{truthfulness} the set of truthfulness conditions of the \Gls{ai}-generated answer,
and therefore \gls{rounds} the set of all possible rounds.\\
Furthermore, let \gls{sessions} denote the set of all valid sessions consisting of three rounds such that no case vignette occurs twice within a session and only one is truthful.
Also, each session pair $(s_1,s_2) \in S^2$ is only allowed to share exactly one case vignette.\\
It follows that \gls{participants} is the set of all valid participant allocations, where each participant is assigned
...
For the purposes of this study, we define a heuristic \sym{abs} that serves as an operational measure of automation bias.
...
Rated Usefulness of \Gls{ai} in mental health vs. \sym{abs}\\
...
The primary objective of the study was to investigate the direct and long-term influence of disclaimer messages on participants' evaluations of \Gls{ai}-generated mental-health recommendations and on the occurrence of \sym{abs}.
...
The initial quality scale is designed to be the core of our \sym{abs} measurement, which would then later be scaled by the subsequent confidence inquiry.
...
The following figures show box plots of the results for \sym{mean-abs} within the active as well as the control group throughout both sessions.
