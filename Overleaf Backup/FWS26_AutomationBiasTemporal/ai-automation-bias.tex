...
We utilized several standardized \gls{ueqplus} scales in order to gain contextual information for each individual  participant. This would allow us to gain insights on the subjective perception of our study design in order to find potential factors that influenced our collected data.
...
We deployed several scales from the \gls{ueqplus} in order to keep track on potential underlying factors that might have influence on the participants' responses.
...
Firstly, as described in the study results \hyperref[res:ueq_control]{\gls{ueqplus}~(\ref*{res:ueq_control})}, there was a notable difference in the control group’s responses from session 1 to session 2, and thus also in comparison to the active group.
...
Furthermore, showing the design intervention lowered the risk handling score \hyperref[res:ueq_risk]{\gls{ueqplus}~(\ref*{res:ueq_risk})}.
