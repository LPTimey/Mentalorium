...
Participants first completed a questionnaire on their attitudes toward \Gls{ai}, reviewed a mental health case vignette, and requested an \gls{ai}-based assessment. After evaluating the generated recommendation, they completed a post-interaction questionnaire.
...
\Gls{ai} is increasingly being integrated into mental health care, where it is used for diagnostic suport, therapy assistance, behavioral monitoring, and clinical decision-making \autocite[]{ali_artificial_2025}.
...
Rated Usefulness of \Gls{ai} in mental health vs. Automation Bias score\\
To establish a potential correlation between attitudes and automation bias scores, we collected a subjective \Gls{ai} usefulness rating which we would later be able to put into relation with the individual bias scores. This would allow us to identify potential patterns between these two scales.

Gender vs. Automation Bias score\\
We applied the same procedure to establish a possible connection between gender identity and the measured automation bias scores.
...
The primary objective of the study was to investigate the direct and long-term influence of disclaimer messages on participants' evaluations of \Gls{ai}-generated mental-health recommendations and on the occurrence of automation bias.
...
The active group had a mean automation bias of 0.45792 in the first session, and a mean automation bias of $0.49458$ in the second session. The control group had a mean automation bias of 0.5618 in the first session, and a mean automation bias of $0.3875$ in the second session.

The following figures show box plots of the results for automation bias within the active as well as the control group throughout both sessions.
...
Firstly, our results suggest that disclaimers informing users about the fallibility of \Gls{ai} can have a positive effect on reducing automation bias.
By explicitly reminding users that \Gls{ai} systems are not infallible, users may be encouraged to critically evaluate \Gls{ai}-generated recommendations instead of accepting them without reflection.
Secondly, our findings do not indicate that this effect persists over the long term, suggesting that a single or short-term exposure to such disclaimers may not be sufficient to produce lasting changes in users’ behavior.
Finally, the results indicate that removing these disclaimers after one week may have negative consequences, potentially leading users to become more reliant on \Gls{ai} recommendations once the reminders are no longer present.
However, the results obtained from the \gls{ueqplus} suggest that disclaimers alone are insufficient to ensure appropriate risk handling when interacting with \Gls{ai} systems. 
Therefore, additional interventions or design strategies may be necessary to effectively support users in maintaining a critical attitude toward \Gls{ai}-generated output. 
Furthermore, this study was conducted with a relatively small sample size, which may have influenced the results and limits the extent to which the findings can be generalized. 
Future research with larger and more diverse participant groups is therefore needed to further validate these findings and explore more effective approaches to mitigating automation bias.
